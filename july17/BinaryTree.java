...
public class BinaryTree {
	...
	public BinaryTree(int[] preo, int[] ino) {
		...
		// root=construct2(posto, ino, psi, pei, isi, iei)
	}
	...
	public void traversalIterative() {// Postorder
	...
	private Node lca(Node node, int d1, int d2) {
		...
		if ((slca != null && llca != null) || (slca != null && rlca != null) || (llca != null && rlca != null)) {
			lca = node;
			found = true;
			return node;
		} else {
	...
	private largestBSTNode largestBSThelper(Node node) {
		if (node == null) {
			largestBSTNode bp = new largestBSTNode();
		...
		largestBSTNode lp = largestBSThelper(node.left);
		largestBSTNode rp = largestBSThelper(node.right);

		largestBSTNode mp = new largestBSTNode();
		...
		if (mp.isBST) {
			...
			if (rp.largestbstsize < lp.largestbstsize) {
